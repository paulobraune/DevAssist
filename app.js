...
// Configurar ambiente (produção ou desenvolvimento)
if (!process.env.NODE_ENV) {
  process.env.NODE_ENV = 'development';
}

// Informar sobre o ambiente atual
console.log('\x1b[36m%s\x1b[0m', `Aplicação iniciada no modo: ${process.env.NODE_ENV.toUpperCase()}`);
if (process.env.NODE_ENV === 'development') {
  console.log('\x1b[33m%s\x1b[0m', 'Modo desenvolvimento: Usando pasta local /files para armazenamento');
} else {
  console.log('\x1b[33m%s\x1b[0m', 'Modo produção: Usando servidor SSH para armazenamento');
}
...
app.use((req, res, next) => {
  ...
  res.locals.appMode = process.env.NODE_ENV;
  next();
});
